...
fn on_wasm_stack<F: FnOnce() -> T + 'static, T: 'static>(
    ...
    //
    // LingoDB local patch: the upstream pool is a process-global
    // `crossbeam_queue::SegQueue<DefaultStack>` (see the original FIXME).
    // For workloads that fire millions of host->guest calls across a fixed
    // worker pool — e.g. one wasm session per LingoDB worker, each running
    // a SQL UDF over a partition — that global queue's head/tail
    // cache-line-bounce becomes the dominant per-call cost above ~4
    // threads. We replace it with a one-slot thread-local cache and only
    // fall back to the global pool on TLS miss / overflow. With one
    // in-flight call per worker (the common case), the global pool is
    // effectively bypassed entirely.
    static STACK_POOL: LazyLock<crossbeam_queue::SegQueue<DefaultStack>> =
        LazyLock::new(crossbeam_queue::SegQueue::new);
    thread_local! {
        static STACK_POOL_TLS: Cell<Option<DefaultStack>> = const { Cell::new(None) };
    }

    let stack = STACK_POOL_TLS
        .with(|c| c.take())
        .or_else(|| STACK_POOL.pop())
        .unwrap_or_else(|| DefaultStack::new(stack_size).unwrap());
    let mut stack = scopeguard::guard(stack, |stack| {
        // Try to keep the freed stack thread-local; if the slot is
        // already occupied (re-entrant call freed first), fall back to
        // the global pool so we never drop a usable stack on the floor.
        if let Some(spilled) = STACK_POOL_TLS.with(|c| c.replace(Some(stack))) {
            STACK_POOL.push(spilled);
        }
    });
